...
def gmail_scanner(request):
    ...
    if request.method == 'POST':
        ...
        if mail_addr and not ',' in mail_addr and '@' in mail_addr:
            msg = ''
            gmail_addr = mail_addr
...
#             final_resp,msg = get_server_response(gmail_addr)
            ...
            for email in data_file:
                ...
                count += 1
                if '@' in msg:
                    msg = 'correct email format'
                else:
                    msg = 'Incorrect email format'
#                 final_resp, msg = get_server_response(gmail_addr)
                msg = gmail_addr + " ====>> " + msg

#                 if not "already associated with an account" in msg:
                if not "@" in msg:
                    continue
                fop.write(gmail_addr)
                fop.write('\n')
