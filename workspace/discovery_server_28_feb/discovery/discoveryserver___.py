...
INTERVAL_TO_DEACTIVATE_CLOUDLET = 30
...
class Heartbeat(threading.Thread):
    ...
    def check_heartbeat(self):
        ...
        thead_list_len = len(threading.enumerate())
        #         if(len(threading.enumerate()) < 3):
        #             thread_name = threading.currentThread().getName()
        #             print "Existing thread ...", thread_name
        #             repr(thread_name).exit()
        delay = self.delay
        inactive_data_dict = {
            'status': 'inactive'
        }

#         print "inside check_heartbeat"
        while(delay):  # maybe check on some other variable
            #         while(thead_list_len > 2):
...
def update_heartbeat_dict():
    ...
    clc_id = request.args.get('clc_id')
    heartbeat_dict[clc_id] = stored_time
...
if __name__ == '__main__':
    ...
    print "before start:", os.getpid()
    heartbeat_t = Heartbeat(HEARTBEAT_INTERVAL)
    heartbeat_t.start()
#     heartbeat_t.setDaemon(True)

    print "after start:", os.getpid()
    discoveryserver.run(
