    ...
    def cloudlets(self, app, country=None, continent=None):
        ...
        if app_policy:
            ...
            for item in cloudlets:
                ...
                memory_capacity = int(cloudlets_capacity[item]['memory'])
                storage_capacity = int(cloudlets_capacity[item]['storage'])
                cpu_capacity = int(cloudlets_capacity[item]['cpu'])

                memory_in_use = int(cloudlet_usage[item]['memory'])
                storage_in_use = int(cloudlet_usage[item]['storage'])
                cpu_in_use = int(cloudlet_usage[item]['cpu'])
                ...
                if available_memory > int(app_requirement['memory']) and \
                        available_cpu > int(app_requirement['cpu']) and \
                        available_storage > int(app_requirement['storage']):
...
if len(sys.argv) < 3:
    print("Usage: %s <app_catalog_ip> <cloudlet_catalog_ip>" % sys.argv[0])
    sys.exit(1)
