...
class EventSubscribers:
    ...
    def __init__(self, subscribers_file):
        ...
        self.subscribers_list = self.parser.sections()

    def notify(self, data_for_subscriber):
        for subs in self.subscribers_list:
            subscribers_url = self.parser.get(subs, 'url')
            try:
                rsp = requests.put(subscribers_url, params=data_for_subscriber)
                print "Response is ::", rsp
            except:
                print subscribers_url, " url not accessible"

    def subscribe(self, subscriber_name, subscriber_url):
        #         import pdb
        #         pdb.set_trace()
        if subscriber_name not in self.subscribers_list:
            self.parser.add_section(subscriber_name)
            self.parser.set(subscriber_name, 'url', subscriber_url)
    ...
    def unsubscribe(self, subscriber_name):
        if subscriber_name in self.subscribers_list:
            self.parser.remove_section(subscriber_name)
            with open(self.file, "w") as cfgfile:
                self.parser.write(cfgfile)
            self.parser.read(self.file)
            self.subscribers_list = self.parser.sections()
...
def discover_cloudlets(*args, **kwargs):
    ...
    # This one is used by httpbin.org as well
...
def register_cloudlets(cloudlet_id):
    try:
        ...
        subscribers.notify(data_for_subscriber)
...
def subscribe():
    try:
        ...
        resp = subscribers.subscribe(subscribing_party, subscribing_party_url)
#         resp = Response(response="%s:SUBSCRIBE_SUCCESSFULL" % subscribing_party, status=httplib.OK)
...
#         subscribing_party_url = request.args.get('url')
        ...
        resp = subscribers.unsubscribe(subscribing_party)
    except:
        resp = Response(
...
subscribers = EventSubscribers(
    "C:/Users/gur40998/workspace/discovery_server/discovery/subscribers.ini")
