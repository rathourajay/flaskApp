...
def discover_cloudlets(developer_id, app_id):
    ...
    print "app_policy_db.cloudlets", app_policy_db.cloudlets(app)
...
#         print "cloudlet_usage:", cloudlet_usage
        ...
        for c in cloudlet_ids:
            usage_dict = Counter(cloudlet_usage[c])
            capacity_dict = Counter(cloudlets_capacity[c])
            available_capacity_dict[c] = dict(capacity_dict - usage_dict)
            if available_capacity_dict[c]['RAM'] >= app_requirement['RAM'] and \
                    available_capacity_dict[c]['CPU'] >= app_requirement['CPU'] and \
                    available_capacity_dict[c]['DISK'] >= app_requirement['DISK']:
                shortlist[c] = cloudlets[c]

#         print "available capacity", available_capacity_dict
        rsp = json.dumps(shortlist)
#         print "resp_shortlist1", rsp
        return rsp
    else:
...
def register_cloudlets():
    ...
    try:
        ...
        if response.status_code == httplib.OK:  # This is 200
            content = "CLOUDLET-REGISTER-OK %s" % (cloudlet_id)
            status = {'code': response.status_code, 'content': content}
            resp = json.dumps(status)
        else:
            content = "CLOUDLET-REGISTER-FAILED %s" % (cloudlet_id)
            status = {'code': response.status_code, 'content': content}
            resp = json.dumps(status)
    except:
        error = "CLOUDLET-DEREGISTER-FAILED %s, Internal Server Error" % cloudlet_id
        return_status = {'code': 500, 'error': error}
...
def deregister_cloudlets():
    ...
    try:
        ...
        response = requests.put(
            "http://localhost:5001/api/CR/change_status", params=data)
        if response.status_code == httplib.OK:  # This is 200
            status = "CLOUDLET-DEREGISTER-OK %s" % (cloudlet_id)
            resp = {'code': response.status_code, 'status': status}
        else:
            status = "CLOUDLET-DEREGISTER-FAILED %s" % (cloudlet_id)
            resp = {'code': response.status_code, 'status': status}
        resp = json.dumps(resp)
    except:
        status = "CLOUDLET-DEREGISTER-FAILED %s, Internal Server Error" % cloudlet_id
        resp = {'code': 500, 'status': status}
        resp = json.dumps(resp)
    return resp
