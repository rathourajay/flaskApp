    ...
    def subscribe(self, subscriber_name, subscriber_url, event_list):
        for event in event_list:
            ...
            if self.parser.has_option(event, subscriber_name):
                return Response(response="%s:ALREADY_SUBSCRIBED" % subscriber_name, status=httplib.OK)
    ...
    def unsubscribe(self, subscriber_name, event_list):
        for event in event_list:
            ...
            if self.parser.has_option(event, subscriber_name):
                ...
                return Response(response="%s:NOT_SUBSCRIBED" % subscriber_name, status=httplib.OK)
...
if len(sys.argv) < 5:
    print("Usage: %s <app_catalog_ip> <cloudlet_catalog_ip> <iam_ip> <configfile_location>" %
          sys.argv[0])
    sys.exit(1)
...
if __name__ == '__main__':
    ...
    INTERVAL_TO_CHECK_HEARTBEAT = int(config_parser.get(
        'discoveryserver', 'INTERVAL_TO_CHECK_HEARTBEAT'))
    INTERVAL_TO_DEACTIVATE_CLOUDLET = int(config_parser.get(
        'discoveryserver', 'INTERVAL_TO_DEACTIVATE_CLOUDLET'))
    SUBSCRIBER_FILE = config_parser.get(
        'discoveryserver', 'SUBSCRIBER_FILE')
