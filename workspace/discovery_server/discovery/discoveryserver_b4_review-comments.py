...
# if country and app_policy["client-regions"] and country not in
# app_policy["client-regions"] and continent not in
# app_policy["client-regions"]:
            ...
            if app_policy["lowLatency"] == "Y":
                if country:
                    ...
                    filter = lambda v: (country in v["regions"][
                        "lowLatency"] or continent in v["regions"]["lowLatency"]) and (v["status"] == "registered") and (v["environment"] == "Production")
                else:
                    # Choose from all low-latency clouds
                    filter = lambda v: (v["regions"]["lowLatency"]) and (
                        v["status"] == "registered") and (v["environment"] == "Production")
            else:
                if country:
                    # Choose from all clouds for specific region
                    filter = lambda v: (country in v["regions"][
                        "all"] or continent in v["regions"]["all"]) and (v["status"] == "registered") and (v["environment"] == "Production")
                else:
            ...
            self.load_db()
            cloudlets = [v['name'] for v in self.db["cloudlets"] if filter(v)]
