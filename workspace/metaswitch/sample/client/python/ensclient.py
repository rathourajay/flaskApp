...
class ENSSession():
    ...
    def __init__(self, app, cloudlet, interface, binding):
        logging.info(
            "Create ENSSession to interface %s on application %s" % (interface, app))
        self.app = app
    ...
    def connect(self):
        ...
        logging.info("Connecting to ENS interface %s at %s" %
                     (self.interface, self.binding))
        try:
...
class ENSClient():
    ...
    class Probe(asyncore.dispatcher):
        ...
        def __init__(self, cloudlet, app):
            ...
            logging.info("Probe cloudlet %s for application %s" %
                         (cloudlet, app))
            self.app = app
            self.cloudlet = cloudlet
        ...
        def handle_read(self):
            ...
            if not self.sampling:
                ...
                if len(self.samples) < 10:
                    ...
                    logging.debug(
                        "Completed 10 RTT probes to %s" % self.cloudlet)
    ...
    def __init__(self, edge_domain, app):
        self.paac = [r[4] for r in socket.getaddrinfo(edge_domain, ENS_PAAC_PORT, 0, socket.SOCK_STREAM) if r[
            0] == socket.AF_INET or r[0] == socket.AF_INET6]

        logging.info("Platform app@cloud locations = %s" %
                     ','.join(["%s:%d" % (c[0], c[1]) for c in self.paac]))
        self.app = app
        self.cloudlet = ""
        self.bindings = {}
        self.probed_rtt = 0.0
    ...
    def init(self):
            ...
            try:
                ...
                try:
                    ...
                    if params[0] == "ENS-INQUIRE-OK":
                        cloudlets = json.loads(
                            rsp.splitlines()[1])["cloudlets"]
                        break
                    else:
        ...
        # Send a service request to platform app@cloud to instantiate the
        # application and microservices.
        for paac in self.paac:
            try:
                s = socket.create_connection(paac)
                logging.debug("TX=>%s: ENS-SERVICE %s %s %s" %
                              (paac[0], self.app, self.cloudlet, self.probed_rtt))
                ...
                try:
                    ...
                    if params[0] == "ENS-SERVICE-OK":
                        ...
                        logging.error(
                            "Failed to initialize application: %s" % rsp)
                        return False
                except:
                    return False
    ...
    def connect(self, interface):
        ...
        session = ENSSession(
            self.app, self.cloudlet, interface, self.bindings[interface])
        if session.connect():
            return session
        else:
            return None
